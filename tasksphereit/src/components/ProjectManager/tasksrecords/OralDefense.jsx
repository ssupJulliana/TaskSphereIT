// src/components/ProjectManager/tasksrecords/OralDefense.jsx
...
// Sample completed Oral Defense tasks record. Add more rows as needed.
const RAW_ROWS = [
  {
    no: 1,
    assigned: "Alejandro F.",
    type: "Presentation",
    task: "Mock Presentation",
    subtask: "Slide Design",
    element: "Software",
    created: "2025-02-10",
    due: "2025-02-14",
    time: "08:00",
...
/** Convert a date string (YYYY-MM-DD) into a human-friendly string. */
...
export default function OralDefense({ onBack }) {
  ...
  // Unique set of project phases for the dropdown filter
  const phases = useMemo(() => {
    const set = new Set(rows.map((r) => r.phase));
    return Array.from(set);
  }, [rows]);

  // Filter rows based on search string and phase filter
  ...
  // Save edited due date/time (increment revision, set status to To Do, remove from record)
  const saveEdit = (no) => {
    setRows((prev) => {
  ...
  return (
    /* Prevent this flex item from expanding and squeezing the sidebar */
    <div className="flex-1 min-w-0 max-w-full overflow-hidden space-y-4">
      ...
      <div className="bg-white border border-neutral-200 rounded-2xl shadow overflow-hidden">
        <div className="overflow-x-auto">
          {/* Give the table a min width so it scrolls instead of pushing/squeezing the sidebar */}
