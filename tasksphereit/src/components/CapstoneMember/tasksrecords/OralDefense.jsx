// src/components/CapstoneMember/tasksrecords/OralDefense.jsx
...
/* =========================== Component =========================== */
 function OralDefense({ onBack, onView }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const snap = await getDocs(collection(db, "oralDefenseTasks"));
        const arr = [];
        snap.forEach((d) => {
          const t = d.data() || {};
        ...
        console.error("Member OralDefense record load failed:", e);
        if (alive) setRows([]);
      }
    })();
    return () => { alive = false; };
  ...
  const handleView = (row) => {
    // Hook this up to your modal or navigation
          ...
          <table className="min-w-[1200px] w-full text-[13px] leading-tight">
            <thead>
              <tr className="text-left text-neutral-500">
                ...
                <th className="py-2 pr-3 whitespace-nowrap">
                  <div className="inline-flex items-center gap-2">
                    <CalendarDays className="w-4 h-4" /> Date Completed
                  </div>
                </th>
